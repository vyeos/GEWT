...
describe("course duration helpers", () => {
  ...
  it("converts semester courses into the expected total years", () => {
    expect(
      getCourseDuration({ duration: 8, duration_type: "semester" }),
    ).toMatchObject({
      totalSemesters: 8,
      totalYears: 4,
      label: "8 semesters",
  ...
  it("labels billing periods as terms for yearly courses and semesters for semester courses", () => {
    expect(
      getCourseBillingPeriods({ duration: 1, duration_type: "year" }),
    ).toEqual([
      { year: 1, semester: 1, label: "Term 1" },
      { year: 1, semester: 2, label: "Term 2" },
    ]);
    expect(
      getCourseBillingPeriods({ duration: 2, duration_type: "semester" }),
    ).toEqual([
  ...
  it("caps totals at the 4-year / 8-semester ceiling the backend bills", () => {
    ...
    expect(
      getCourseDuration({ duration: 5, duration_type: "year" }),
    ).toMatchObject({
      totalYears: 4,
      totalSemesters: 8,
      label: "5 years",
    ...
    expect(
      getCourseDuration({ duration: 12, duration_type: "semester" }),
    ).toMatchObject({
      totalYears: 4,
      totalSemesters: 8,
      label: "12 semesters",
  ...
  it("clamps current periods and derives the current course year", () => {
    expect(
      getCurrentCoursePeriod(makeStudent({ current_course_period: 0 })),
    ).toBe(1);
    expect(
      getCurrentCoursePeriod(makeStudent({ current_course_period: 99 })),
    ).toBe(6);
    expect(
      getCurrentCourseYear(makeStudent({ current_course_period: 3 })),
    ).toBe(2);
    expect(
      getBillingThroughPeriod(makeStudent({ current_course_period: 3 })),
    ).toBe(4);
  });
  ...
  it("formats ordinals and period labels", () => {
    ...
    expect(
      formatCoursePeriod(makeStudent({ course_duration_type: "semester" }), 4),
    ).toBe("Semester 4");
    expect(
      formatCoursePeriod(makeStudent({ course_duration_type: "year" }), 4),
    ).toBe("Term 4");
    expect(
      formatCourseYearPeriodRange(makeStudent({ current_course_period: 3 }), 2),
    ).toBe("2nd Year (Terms 3-4)");
  });
});
